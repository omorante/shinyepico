...
This report was created with the ÉPICo ShinyApp.<br>
...
=======
   snps = getSnpBeta(params$rval_rgset)
>>>>>>> 707ab8e45d439906f6ffada0effac4de890932c8
...
###(Heatmap with all contrasts and comparison)

```{r filtered_table_all,echo=FALSE}
<<<<<<< HEAD
...
=======
  DT::datatable(rbindlist(params$rval_filteredlist, idcol="contrast") %>% mutate(type=ifelse(dif_current > 0, "Hypermethylated", "Hypomethylated")) %>% group_by(contrast, type) %>% summarise(CpGs=dplyr::n()) %>% pivot_wider(names_from=type, values_from=CpGs) %>% mutate(total = Hypermethylated + Hypomethylated))
>>>>>>> 707ab8e45d439906f6ffada0effac4de890932c8
...
=======
>>>>>>> 707ab8e45d439906f6ffada0effac4de890932c8
* Column dendogram: __TRUE__
* Scale: __`r params$scale`__
* Cluster Algorithm: __`r params$clusteralg`__
* Distance: __`r params$distance`__
```{r all_heatmap, echo=FALSE, message=FALSE, warning=FALSE}

 create_heatmap(params$rval_filteredlist, as.data.frame(gset_beta), factorgroups =  factor(rval_voi), groups2plot = rval_voi %in% rval_voi, contrasts2plot = params$rval_contrasts %in% params$rval_contrasts, Colv = T, clusteralg = params$clusteralg, distance = params$distance, scale = params$scale, static=F)

```


###(Custom heatmap)
```{r filtered_table,echo=FALSE}
<<<<<<< HEAD
...
=======
  DT::datatable(rbindlist(params$rval_filteredlist[params$rval_contrasts %in% params$contrasts2plot], idcol="contrast") %>% mutate(type=ifelse(dif_current > 0, "Hypermethylated", "Hypomethylated")) %>% group_by(contrast, type) %>% summarise(CpGs=dplyr::n()) %>% pivot_wider(names_from=type, values_from=CpGs) %>% mutate(total = Hypermethylated + Hypomethylated))
>>>>>>> 707ab8e45d439906f6ffada0effac4de890932c8
...
create_heatmap(params$rval_filteredlist, as.data.frame(gset_beta), factorgroups =  factor(rval_voi[rval_voi %in% params$groups2plot], levels=params$groups2plot),groups2plot = rval_voi %in% params$groups2plot, contrasts2plot = params$rval_contrasts %in% params$contrasts2plot, Colv = as.logical(params$Colv), clusteralg = params$clusteralg, distance = params$distance, scale = params$scale, static=F)
