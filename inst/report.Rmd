...
```{r snps_heatmap, echo=FALSE}
   ...
   heatmaply::heatmaply(snps, col = colors.martin, Colv = TRUE, key.title = "", na.rm = TRUE, dendogram = "both", scale = "row", 
                       col_side_colors = params$rval_sheet_target[,params$donor_var, drop=FALSE], distfun = "pearson", hclustfun = function(x) hclust(x, method = "average"),
...
```{r filtered_table_all,echo=FALSE,message=FALSE, warning=FALSE}
  DT::datatable(data.table::rbindlist(params$rval_filteredlist, idcol="contrast") %>% 
    ...
    dplyr::mutate(dplyr::across(c("Hypermethylated","Hypomethylated", "total"), round))
  )
...
```{r filtered_table,echo=FALSE,message=FALSE, warning=FALSE}
  DT::datatable(data.table::rbindlist(params$rval_filteredlist[params$rval_contrasts %in% params$contrasts2plot], idcol="contrast") %>% 
        dplyr::mutate(type = factor(ifelse(.data$dif_current < 0, "Hypermethylated", "Hypomethylated"), levels = c("Hypermethylated","Hypomethylated"))) %>% 
        dplyr::group_by(.data$contrast, .data$type)  %>%
        dplyr::summarise(CpGs = dplyr::n()) %>% 
        tidyr::complete(.data$contrast, .data$type, fill=list(CpGs=0)) %>%
