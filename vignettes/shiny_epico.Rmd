...
Usually, iDATs and sample sheet are obtained meeting these features by default, and no additional work is required.
...
On the one hand, the QC Signal plot shows the median methylated (mMed) and umethylated (uMed) of each sample array. When mMed or uMed of a sample is less than 10, it is considered "Suboptimal". Depending on whether the signal is very far from this threshold and depending on the rest of results, these types of samples should be excluded from the analysis.


On the other hand, the bisulfite conversion plot shows the minimum ratio of converted/non-converted signal of bisulfite conversion II probes for each sample. This ratio should be higher than 1.5.



### Density plot and boxplot

A correct array normalization is essential for the results of the subsequent steps in the workflow. shinyÉPICo uses the normalization methods available in the minfi package:

- __Raw__ (without normalization)
- __Illumina__ (an implementation of Genome Studio normalization)
...
- Additionally, it offers the option of performing Noob within array normalization followed by Quantile normalization (__Noob + Quantile__), analogously to the Funnorm method. This non-standard approach has empirically shown good results in our experience.

For details about the methods, the minfi documentation and the respective publications are very good references. Even though depending on the experimental design and the type of changes expected a normalization method can be better than another, usually the best method should be determined empirically from the valid options.

shinyÉPICo makes very simple to test different methods. When a normalization method is selected, the different charts are automatically generated. When a method is chosen, clicking the "Select" button.
